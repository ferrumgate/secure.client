...
const createTray = () => {
    tray = new Tray(path.join(assetsDirectory, 'img', 'logo-red.png'))
    const contextMenu = Menu.buildFromTemplate([
        {
            label: 'Connect', type: 'normal', icon: path.join(assetsDirectory, 'img', 'connect.png')
        },
        {
            label: 'Disconnect', type: 'normal', visible: true,
            icon: path.join(assetsDirectory, 'img', 'disconnect.png')
        },
        {
            label: 'Options', type: 'normal',
            icon: path.join(assetsDirectory, 'img', 'settings.png'), click: () => { toggleWindow() }
        },
        {
            label: 'Quit', type: 'normal',
            icon: path.join(assetsDirectory, 'img', 'close.png')
        },
...
ipcMain.on('weather-updated', (event, weather) => {
    ...
    switch (weather.currently.icon) {
        ...
        case 'partly-cloudy-night':
            tray.setImage(path.join(assetsDirectory, 'img', 'cloudTemplate.png'))
            break
        case 'rain':
        case 'sleet':
        case 'snow':
            tray.setImage(path.join(assetsDirectory, 'img', 'umbrellaTemplate.png'))
            break
        case 'clear-night':
            tray.setImage(path.join(assetsDirectory, 'img', 'moonTemplate.png'))
            break
        case 'wind':
            tray.setImage(path.join(assetsDirectory, 'img', 'flagTemplate.png'))
            break
        case 'clear-day':
        default:
            tray.setImage(path.join(assetsDirectory, 'img', 'sunTemplate.png'))
    }
})